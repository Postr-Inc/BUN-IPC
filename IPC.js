...
class IPC {
   ...
   * @returns
   */
 newServer(proto = 0, config = { port: 3434 }) {
    ...
    return {
      ...
      Console: {
        log: (...msg) => {
            messages.push(msg)
            sendMsg({ CONSOLE: true, LOG: true, msg });
        },
        error: (errorLevel, ...msg) => {
      ...
      /**
       * @description Set a custom Protocol Listener ID
       * @param {number} type 
       */
      set_IPC_Type: (type) => {
            proto = type
            console.log(`Switching to protocol ${type}`)
  ...
  client(config = { use: this.typeEnums.DFT, port: 0, callback: () => {} }) { 
    ...
    return {
      ...
      Console: {
        /**
         * @type {console.log}
         * @description Read from the console
